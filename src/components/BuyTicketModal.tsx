            ...
            <div className="p-6 sm:p-8">
                ...
                <>
                  ...
                  <div className="mt-7">
                    ...
                    <div className="flex items-center justify-between mt-2 p-2 rounded-xl"
                      ...
                      <button onClick={() => setQuantity(q => Math.max(1, q - 1))} disabled={quantity <= 1}
                        className="w-10 h-10 rounded-lg flex items-center justify-center text-white disabled:opacity-30 hover:bg-white/5 transition">
                        <Minus size={16} />
                      </button>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="relative mt-2">
                      ...
                      <input type="text" value={name} onChange={e => setName(e.target.value)}
                        ...
                        className="w-full pl-10 pr-3 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:outline-none focus:border-white/30 transition" />
                    </div>
                  </div>
                  ...
                  {error && (
                    <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
                      className="mt-3 text-sm text-red-400 text-center">{error}</motion.p>
                  )}
                  ...
                  <motion.button onClick={handleProceedToPayment}
                    ...
                    style={{ background: `linear-gradient(135deg, ${artist.accent}, ${artist.accent}cc)` }}>
                    {loadingIntent
                      ? <><Loader2 size={18} className="animate-spin" /> Φόρτωση...</>
                      : <><Ticket size={18} /> Συνέχεια στην Πληρωμή</>}
                  </motion.button>
                </>
              )}
              ...
              {step === 'payment' && clientSecret && (
                <Elements stripe={stripePromise} options={{
                  ...
                  appearance: {
                    theme: 'night',
                    variables: { colorPrimary: artist.accent, borderRadius: '12px' }
                  }
                }}>
                  <CheckoutForm
                    artist={artist}
