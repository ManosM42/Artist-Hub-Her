    ...
    <section id="about" className="relative py-28 bg-transparent overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          {/* LIQUID GLASS CONTAINER FOR TEXT ELEMENTS */}
          <motion.div 
            initial={{ opacity: 0, x: -40 }} 
            whileInView={{ opacity: 1, x: 0 }} 
            viewport={{ once: true, margin: '-60px' }} 
            transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
            className="p-8 sm:p-10 rounded-3xl backdrop-blur-xl border border-white/5"
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }} className="flex items-center gap-6">
              <div className="flex -space-x-3">
                {['https://images.pexels.com/photos/1699161/pexels-photo-1699161.jpeg?auto=compress&cs=tinysrgb&w=80', 'https://images.pexels.com/photos/1763075/pexels-photo-1763075.jpeg?auto=compress&cs=tinysrgb&w=80', 'https://images.pexels.com/photos/1540406/pexels-photo-1540406.jpeg?auto=compress&cs=tinysrgb&w=80'].map((src, i) => (
                  <img key={i} src={src} alt="fan" className="w-10 h-10 rounded-full border-2 border-black/40 object-cover" />
                ))}
              </div>
              <div>
                <p className="text-white font-bold text-sm">50,000+ fans</p>
                <p className="text-gray-400 text-xs">trust Heraklion is Alive</p>
              </div>
            </motion.div>
          </motion.div>
            ...
            {features.map((feature, i) => {
              ...
              return (
                <motion.div 
                  ...
                  className="group p-5 rounded-2xl transition-all duration-300 backdrop-blur-md" 
                  // Updated card styles for a cleaner glass effect over dynamic backgrounds
                ...
                >
                  ...
                  <p className="text-gray-300 text-xs leading-relaxed">{feature.text}</p>
                </motion.div>
              );
            })}
          </motion.div>
